...
public class DKCamera: UIViewController, RMImagePickerControllerDelegate {
    ...
    public var imageViewButton : UIButton!
    let pickerController = DKImagePickerController()
    var nearByPlaceObjectId : String?
    ...
    public func setupUI() {
        ...
        let imageViewButton: UIButton = {
            let imageViewButton = UIButton()
            imageViewButton.addTarget(self, action: #selector(DKCamera.buttonAction), forControlEvents: .TouchUpInside)
            //            imageViewButton.setImage(DKCameraResource.cameraSwitchImage(), forState: .Normal)
            imageViewButton.layer.borderColor = UIColor.whiteColor().CGColor
            imageViewButton.contentMode = UIViewContentMode.ScaleAspectFill
            imageViewButton.imageView?.contentMode = UIViewContentMode.ScaleAspectFill
            imageViewButton.layer.borderWidth = 1
        ...
        imageViewButton.frame = CGRectMake(10, 5, 50, self.bottomView.frame.size.height - 10)
        imageViewButton.autoresizingMask = [.FlexibleLeftMargin, .FlexibleTopMargin, .FlexibleBottomMargin]
        bottomView.addSubview(imageViewButton)
        ...
        let captureButton: UIButton = {
            ...
            captureButton.setBackgroundImage(UIImage(named: "CameraButton"), forState: UIControlState.Normal)
        ...
        let doneButton: UIButton = {
            let doneButton = UIButton()
            doneButton.addTarget(self, action: #selector(DKCamera.dismiss), forControlEvents: .TouchUpInside)
        ...
        doneButton.frame.origin = CGPoint(x: bottomView.bounds.width - doneButton.bounds.width - 15,
                                          y: (bottomView.bounds.height - doneButton.bounds.height) / 2)
        doneButton.autoresizingMask = [.FlexibleBottomMargin, .FlexibleLeftMargin]
        ...
        let cancelButton: UIButton = {
            let cancelButton = UIButton()
            cancelButton.addTarget(self, action: #selector(DKCamera.dismissCamera), forControlEvents: .TouchUpInside)
            //			cancelButton.setImage(DKCameraResource.cameraCancelImage(), forState: .Normal)
        ...
        cancelButton.frame.origin = CGPoint(x: bottomView.bounds.width - cancelButton.bounds.width - 20,
                                            y: 20)
            ...
            dispatch_async(dispatch_get_global_queue(0, 0), {
                ...
                stillImageOutput.captureStillImageAsynchronouslyFromConnection(connection, completionHandler: { (imageDataSampleBuffer, error: NSError?) -> Void in
                    ...
                    if error == nil {
                        ...
                        self.imageViewButton.setImage(UIImage(data: imageData), forState: UIControlState.Normal)
    ...
    public func rmImagePickerController(picker: RMImagePickerController, didFinishPickingAssets assets: [PHAsset]) {
        self.gallaryAssets.appendContentsOf(assets)
        self.dismissPickerPopover()
    }
    
    public func rmImagePickerControllerDidCancel(picker: RMImagePickerController) {
        self.dismissPickerPopover()
    }
