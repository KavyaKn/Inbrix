		...
		for group in self.groups!.values {
			if let changeDetails = changeInstance.changeDetailsForObject(group.originalCollection) {
				if changeDetails.objectWasDeleted {
					...
					self.notifyObserversWithSelector(Selector("groupDidRemove(_:)"), object: group.groupId)
					continue
				}
				...
				if let objectAfterChanges = changeDetails.objectAfterChanges as? PHAssetCollection {
					...
					self.notifyObserversWithSelector(Selector("groupDidUpdate(_:)"), object: group.groupId)
				}
			}
			...
			if let changeDetails = changeInstance.changeDetailsForFetchResult(group.fetchResult) {
				if let removedIndexes = changeDetails.removedIndexes {
					...
					self.notifyObserversWithSelector(Selector("group(_:didRemoveAssets:)"), object: group.groupId, objectTwo: removedAssets)
				}
				...
				if changeDetails.insertedObjects.count > 0  {
					...
					self.notifyObserversWithSelector(Selector("group(_:didInsertAssets:)"), object: group.groupId, objectTwo: insertedAssets)
				}
			}
		}
	}
